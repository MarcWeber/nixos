...
let
  ...
  backupHilfe = pkgs.writeScriptBin "backup-storebackup-hilfe" ''
  #!/bin/sh
  cat << EOF
  Steps Backup machen:
  ===================
  ...
  sshMawercer = pkgs.writeScriptBin "zugriff-fuer-marc" ''
  #!/bin/sh
  cat << EOF
  ...
  3) Ausführen, Passwort was du von Marc bekommst eingeben. Dies veröffentlicht
     deinen Port 22 as Port 8012 auf mawercer.de:

    "ssh -NR\*:8012:localhost:22 matthias_mundelfingen@mawercer.de"
...

{
  ...
  services.mingetty.helpLine = pkgs.lib.mkOverride 50
    ''
      ...
      Dieser Befehl erinnert dich an alle Details:

        backup-storebackup-hilfe (vermutlich nimm das ?)
        backup-bup-hilfe

      Dieser Befehl erkährt, wie du SSH Zugriff veröffentlichen kannst, so dass
      sich z.B Marc einloggen kann.
